...
6. 라우터에서 실행되고 있는 DHCP 서버가 CIDR 블록 ```68.85.2.0/24```에 있는 IP 주소들을 할당할 수 있다고 가정
   ...
   - 이더넷 프레임은 출발지 MAC 주소로 홈 네트워크로의 라우터 인터페이스의 MAC 주소(```00:22:6B:45:1F:1B```)를, 목적지 MAC 주소로 랩톱의 MAC 주소(```00:16:D3:23:68:8A```)를 가짐
...
5. 게이트웨이 라우터는 학교 네트워크로의 인터페이스를 통해 ARP 질의 메세지를 포함하고 있는 프레임 수신
   ...
   - ARP 응답 메세지를 목적지 주소 ```00:16:D3:23:68:8A```(랩톱)인 이더넷 프레임에 넣어 스위치로 보내며, 스위치는 프레임을 랩톱으로 전달
